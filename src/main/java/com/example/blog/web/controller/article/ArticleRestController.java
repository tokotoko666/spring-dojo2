...
public class ArticleRestController implements ArticlesApi {
    ...
    @Override
    public ResponseEntity<ArticleDTO> creteArticle(ArticleForm form) {
    ...
    public ResponseEntity<ArticleCommentDTO> createComment(Long articleId, ArticleCommentForm form) {
        ...
        var newComment = articleCommentService.create(loggedInUser.getUserId(), articleId, form.getBody());

        var body = ArticleCommentMapper.toDTO(newComment);

        var location = UriComponentsBuilder.fromPath("/articles/{articleId}/comments/{commentId}")
